...
def protocol_head(access):
    out_str = ""
    data = [
        " номер | Кол-во КТ опорного | Кол-во КТ обл. | Кол-во | Кол-во общих КТ | Вариант | Положение на ",
        " п/п | изображения | видимости | общих КТ | после фильтра | искажения | опорном изображ. "
    ]
...
main_path = 'C:\\My\\Projects\\images\\main\\WK_00004-1.jpg'          # Опорное изображение
...
height = 400                                # Высота снимка опорного изображения
...
width = [7, 20, 16, 10, 17, 20, 20]         # Ширина столбцов протокола
...
for i in range(4):
    ...
    while augmentation_index < len(augment):
        ...
        while y2 <= main_crop_img.shape[0]:
            ...
            test = cmp.Compare(kp, des, height, big_map.shape, crop_img, flight_altitude, big_map, all_iter)
            ...
            data_compare[6] = "не найдено" if data_compare[6] == None else data_compare[6]
            # data_compare.insert(6, fluctuation)                     # добавление отклонения найденного местоположения от реального
            # data_compare.pop(7)

            # if augmentation_index == 0 and fluctuation != "не найдено":
            #     local_orig_abs_fluct[0] += abs(fluctuation[0])
            #     local_orig_abs_fluct[1] += abs(fluctuation[1])
            # elif augmentation_index > 0 and fluctuation != "не найдено":
            #     local_abs_fluct[0] += abs(fluctuation[0])
            #     local_abs_fluct[1] += abs(fluctuation[1])
